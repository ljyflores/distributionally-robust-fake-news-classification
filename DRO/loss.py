...
wandb.init(project="sds632", entity="ljyflores_team")
...
class LossComputer:
    ...
    def loss(self, yhat, y, group_idx=None, is_training=False):
        ...
        # group_acc,  group_count = self.compute_group_avg(per_sample_losses, group_idx)
        group_acc = group_loss
        ...
        if is_training:
            wandb.log({"Average Loss": actual_loss})
            for (i,l) in enumerate(group_loss):
                wandb.log({f"Group {i} Loss": l})
        # self.update_historical(actual_loss, group_loss)
        
        return actual_loss

    # def update_historical(self, actual_loss, group_loss):
    #     self.hist_loss.append(actual_loss)
    #     self.hist_group_loss.append(group_loss)
    ...
    def compute_group_avg(self, losses, group_idx):
        ...
        group_loss = torch.matmul(group_map, losses.view(-1).float())/group_denom
